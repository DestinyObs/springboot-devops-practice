...
export DATABASE_URL=jdbc:mysql://prod-db:3306/user_registration_db
...
docker build -t user-registration-service:prod .

# Run in production mode
docker run -d \
  --name user-registration-prod \
  -p 8080:8080 \
  --env-file .env.prod \
  user-registration-service:prod
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: app
        image: user-registration-service:prod
        ports:
        - containerPort: 8080
        env:
...
docker build -t user-registration-service .
docker tag user-registration-service:latest 123456789012.dkr.ecr.us-east-1.amazonaws.com/user-registration-service:latest
docker push 123456789012.dkr.ecr.us-east-1.amazonaws.com/user-registration-service:latest
```

**Google Cloud Run:**
```bash
# Build and deploy to Cloud Run
gcloud builds submit --tag gcr.io/PROJECT_ID/user-registration-service
gcloud run deploy --image gcr.io/PROJECT_ID/user-registration-service --platform managed
```
...
3. **Logging**
   ...
   logging:
     pattern:
       console: '{"timestamp":"%d","level":"%level","logger":"%logger","message":"%message"}%n'
   ```
